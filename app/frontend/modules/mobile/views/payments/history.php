...
?>
<form id="paymentsHistoryForm" class="payments-history-form -mobile-padding">
	<div class="payments-history-form_date">
		<label for="paymentHistoryDateFrom" class="mts-date-field -history"><input type="text" id="paymentHistoryDateFrom" class="mts-date-field_input" name="filter[dateFrom]" value="<?= $searchModel->dateFrom ?>"></label>
		-
		<label for="paymentHistoryDateTo" class="mts-date-field -history"><input type="text" id="paymentHistoryDateTo" class="mts-date-field_input" name="filter[dateTo]" value="<?= $searchModel->dateTo ?>"></label>
	</div>
...
$settings = [
	...
	'itemOptions' => ['class' => 'mts-list-view_item --list-item'],
	'emptyText' => 'У Вас нет ни одного платежа, соответствующего выбранным параметрам.',
	'emptyTextOptions' => ['class' => 'empty-text'],
	'pager' => [
		'class' => \frontend\modules\mobile\components\widgets\LinkPager::className(),
		'options' => ['class' => 'pagination -mobile-padding', 'id' => 'historyPagination'],
	],
];
?>
