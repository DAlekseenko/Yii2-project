...
class NestedSetsBehavior extends Behavior
{
    ...
    public function beforeInsert()
    {
        ...
        switch ($this->operation) {
            ...
            case self::OPERATION_PREPEND_TO:
                $this->beforeInsertNode($this->node->getAttribute($this->leftAttribute) + 1, 1, $this->node->getPrimaryKey());
                break;
            case self::OPERATION_APPEND_TO:
                $this->beforeInsertNode($this->node->getAttribute($this->rightAttribute), 1, $this->node->getPrimaryKey());
                break;
            case self::OPERATION_INSERT_BEFORE:
                $this->beforeInsertNode($this->node->getAttribute($this->leftAttribute), 0, $this->node->getAttribute($this->parentAttribute));
                break;
            case self::OPERATION_INSERT_AFTER:
                $this->beforeInsertNode($this->node->getAttribute($this->rightAttribute) + 1, 0, $this->node->getAttribute($this->parentAttribute));
                break;
            default:
     ...
     * @throws Exception
     */
    protected function beforeInsertNode($value, $depth, $parentId)
    {
        ...
        $this->owner->setAttribute($this->parentAttribute, $parentId);
    ...
    public function afterDelete()
    {
        ...
        if ($this->owner->isLeaf() || $this->operation === self::OPERATION_DELETE_WITH_CHILDREN) {
            ...
            $condition = [
                ...
                ['>', $this->leftAttribute, $this->owner->getAttribute($this->leftAttribute)],
                ['<', $this->rightAttribute, $this->owner->getAttribute($this->rightAttribute)],
                ['=', $this->depthAttribute, $this->owner->getAttribute($this->depthAttribute) + 1]
            ];
            $this->applyTreeAttributeCondition($condition);
